...
/*   Updated: 2024/10/23 12:42:05 by alaakson         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int read_map(t_game *game, const char *file_path) {
    int fd;
    char *line;
    size_t row;
    ...
    if (close(fd) < 0) {
        ft_printf("Failed to close map file\n");
        return -1; // Indicate failure
    }

    // Check if any rows were read
    if (row == 0) {
        ft_printf("Error: No rows read from map file\n");
        return -1; // Indicate no rows were read
    }
...
void load_map(t_game *game, const char *file_path) {
    ...
    if (read_map(game, file_path) < 0) {
        // Free memory allocated for the map rows if read_map fails
